...
test('shows progress bar when showProgress=true', t => {
	...
	t.true(
		output.includes('█') || output.includes('░') || output.includes('50%'),
	);
});
...
test('hides progress bar when showProgress=false', t => {
	...
	const {lastFrame} = render(
		<TodosDisplay todos={todos} showProgress={false} />,
	);
	const output = lastFrame();
...
test('shows pending todos second', t => {
	const todos = [Todo.completed('Last'), Todo.pending('Middle')];
	const {lastFrame} = render(<TodosDisplay todos={todos} />);
	const output = lastFrame();
...
test('shows count for each status section', t => {
	const todos = [Todo.pending('P1'), Todo.pending('P2'), Todo.inProgress('I1')];
	const {lastFrame} = render(<TodosDisplay todos={todos} />);
	const output = lastFrame();
...
test('handles all todos completed', t => {
	const todos = [Todo.completed('Done 1'), Todo.completed('Done 2')];
	const {lastFrame} = render(<TodosDisplay todos={todos} />);
	const output = lastFrame();
...
test('handles all todos pending', t => {
	const todos = [Todo.pending('Todo 1'), Todo.pending('Todo 2')];
	const {lastFrame} = render(<TodosDisplay todos={todos} />);
	const output = lastFrame();
